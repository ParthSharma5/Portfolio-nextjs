...
const Certification = ({ filter }) => {
  ...
  return (
    <section className="min-h-screen py-8 sm:py-12 lg:py-16 px-4 sm:px-6 lg:px-8
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {filteredCerts.length === 0 ? (
          <motion.div
            ...
            className="text-center py-16"
          >
            <div className="dark:bg-gray-800/50 bg-white/70 backdrop-blur-sm rounded-2xl p-8 max-w-md mx-auto border dark:border-gray-700 border-gray-200">
              <FiAward className="dark:text-gray-500 text-gray-400 text-5xl mx-auto mb-4" />
              <h3 className="text-xl font-semibold dark:text-white text-gray-900 mb-2">No Certifications Found</h3>
            ...
            {filteredCerts.map((cert, index) => (
              <motion.div
                ...
                className="group
                  ...
                  rounded-2xl shadow-lg hover:shadow-2xl hover:shadow-indigo-500/20
                  transition-all duration-500 overflow-hidden"
              >
                {/* Card Header */}
                <div className="relative bg-gradient-to-r from-indigo-500/10 to-purple-500/10 p-6 border-b dark:border-gray-700 border-gray-200">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <FiAward className="text-indigo-400 text-2xl flex-shrink-0 mt-1" />
                    <motion.span
                      initial={{ scale: 0.8, opacity: 0 }}
                  ...
                  <h3 className="text-xl font-bold dark:text-white text-gray-900 group-hover:text-indigo-300 transition-colors duration-300 line-clamp-2">
                    {cert.title}
                  </h3>
                </div>
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <p className="text-xs dark:text-gray-400 text-gray-500 uppercase tracking-wide">Issuer</p>
                      <p className="dark:text-white text-gray-900 font-medium">{cert.issuer}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <p className="text-xs dark:text-gray-400 text-gray-500 uppercase tracking-wide">Year</p>
                      <p className="dark:text-white text-gray-900 font-medium">{cert.year}</p>
                    </div>
                  </div>

                  {/* Additional Info */}
                  {(cert.duration || cert.skills) && (
                    <div className="space-y-3 mb-6 pt-4 border-t dark:border-gray-700 border-gray-200">
                      {cert.duration && (
                        <div>
                          <p className="text-xs dark:text-gray-400 text-gray-500 uppercase tracking-wide mb-1">Duration</p>
                          <p className="dark:text-white text-gray-900 text-sm">{cert.duration}</p>
                        </div>
                      )}
                      {cert.skills && (
                        <div>
                          <p className="text-xs dark:text-gray-400 text-gray-500 uppercase tracking-wide mb-2">Skills</p>
                          <div className="flex flex-wrap gap-2">
                            {cert.skills.slice(0, 3).map((skill, i) => (
                              <span
                                key={i}
                                className="px-2 py-1 dark:bg-gray-700/50 bg-gray-100 dark:text-gray-300 text-gray-700 text-xs rounded-full border dark:border-gray-600/50 border-gray-300"
                              >
                                {skill}
                              </span>
                            ))}
                            {cert.skills.length > 3 && (
                              <span className="px-2 py-1 dark:bg-gray-700/50 bg-gray-100 dark:text-gray-400 text-gray-500 text-xs rounded-full">
                                +{cert.skills.length - 3} more
                              </span>
                            )}
                          </div>
                  ...
                  <div className="flex gap-3">
                    {cert.link ? (
                      ...
                      <button
                        disabled
                        className="flex items-center justify-center gap-2 dark:bg-gray-700 bg-gray-200 dark:text-gray-400 text-gray-500 px-4 py-3 rounded-lg font-semibold transition-all duration-300 flex-1 text-center cursor-not-allowed"
                      >
                        <FiAward className="w-4 h-4" />
                        Not Available
                      </button>
        ...
        {filteredCerts.length > 0 && (
          <motion.div
            ...
            className="mt-12 sm:mt-16 text-center"
          >
            <div className="dark:bg-gray-800/50 bg-white/70 backdrop-blur-sm rounded-2xl p-6 border dark:border-gray-700 border-gray-200 max-w-2xl mx-auto">
